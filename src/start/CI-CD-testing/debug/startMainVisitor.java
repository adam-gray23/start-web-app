...
public class startMainVisitor extends startBaseVisitor<Object>{
    ...
    int numOfStops = 0;
    public startMainVisitor(String fileName){
        //get line one of the file, it should be a number of stops
        try {
        ...
        //line 2 is an array of breakpoints, add them to the arraylist
        try {
            File file = new File(fileName);
            Scanner scanner = new Scanner(file);
            scanner.nextLine();
            //while the next line is not a blank line
            while (scanner.hasNextLine()) {
                String line = scanner.nextLine();
                //add every line to the arraylist, the lines are read as strings
                if (!line.equals("")){
                    //add the line to the arraylist
                    breakPointArr.add(Integer.parseInt(line));
                }
                else{
                    break;
                }
        ...
        //save every other line to the arraylist
        try {
            File file = new File(fileName);
            Scanner scanner = new Scanner(file);
            while (scanner.hasNextLine()) {
    ...
    public void checkOutputAndNumOfStops(){
        //check if numOfStops is same as totalStops
        if (numOfStops != totalStops){
            System.out.println("Error: Number of stops does not match number of breakpoints!");
            System.out.println("Number of stops: " + numOfStops);
            System.out.println("Number of breakpoints: " + totalStops);
            System.exit(0);
        }
        //check if the arraylist is empty
        if (arrOutput.size() == 0){
            System.out.println("Error: Output file is empty!");
            System.exit(0);
        }
    ...
    ArrayList<Integer> breakPointArr = new ArrayList<Integer>();
    int totalStops = 0;
    public void breakpoint(int line){
        //everytime we hit a breakpoint, increase number of stops
        totalStops++;
    }
    ...
    public Object visitProgram(startParser.ProgramContext ctx) {
        ...
        scanner.close();
        //check if the output file and number of stops match
        checkOutputAndNumOfStops();
        return null;
    }
...
public Object visitCompExpression(startParser.CompExpressionContext ctx){
    ...
    public Object visitPrint_statement(startParser.Print_statementContext ctx) {
        ...
        //check if out is the same as the first element in the arraylist
        if (arrOutput.size() > 0){
            if (out.equals(arrOutput.get(0))){
